...
{
    if (node == nullptr || node == block) // if node == block return
        return;
...
void printKNodesFar(Node *node, int data, int k)
{
    ...
    {
        printKLevelsDown(path[i], k - i, i == 0 ? NULL : path[i - 1]); // klevelsdown - i of path
        // for i==0 : no blocker but i == 1 : path[0] is blocker and so on.
    }
}
