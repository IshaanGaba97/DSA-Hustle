...
int addListHelper(Node* one, int p1, Node* two, int p2, linkedlist &res){
    if(one==NULL && two==NULL){
        return 0;
    }
    //oc-oldcarry nd-newdata nc-new carry
    int oc = 0;
    int data = 0;
    if(p1 > p2){
         oc =addListHelper(one->next, p1-1, two, p2, res);
        data = one->data + oc;
    }else if (p1 < p2){
         oc = addListHelper(one, p1, two->next, p2-1, res);
        data = two->data + oc;
    }else {
         oc = addListHelper(one->next, p1-1, two->next, p2-1, res);
