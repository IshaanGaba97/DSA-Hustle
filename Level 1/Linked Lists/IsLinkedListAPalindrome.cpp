...
class LinkedList{
    ...
    bool IsPalindromeHelper(Node* right){
        ...
        if(rres==false){                    //if they are not equat than false
            return false;
        }else if(pleft->data != right->data){    //check the current left & right
            return false;
        }else{
            pleft = pleft->next;     //left is increasing for next increased right
            return true;
        }
    }
    
    Node* pleft;            //pleft in heap
